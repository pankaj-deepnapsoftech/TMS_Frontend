...
import { Calendar, Clock, Flag, User, Briefcase, Users } from 'lucide-react';
                  ...
                  {assignedUsers.map(assignedUser => (
                    ...
                    >
                      <Avatar className="w-8 h-8">
                        ...
                        <AvatarFallback className="text-xs bg-purple-500/20">{getInitials(assignedUser.name)}</AvatarFallback>
                      </Avatar>
                      <div>
                        <p className={`font-medium text-sm ${
                          assignedUser.id === user?.id ? 'text-purple-200' : 'text-white'
                        }`}>
                          {assignedUser.id === user?.id ? 'You' : assignedUser.name}
                        </p>
                        <p className="text-xs text-gray-400">
                          {departments.find(d => d.id === assignedUser.departmentId)?.name || 'No Department'}
                        </p>
                      </div>
                    </motion.div>
                  ))}
